...
for test in tests:
    ...
    assert grad == test[
        'gradient_out'], f"Compute Gradient broke on {test['num_inputs']} inputs {grad} should be equal to {test['gradient_out']}"
    minimizer.descend(scaling_factor=0.001, delta=0.01,
                      num_steps=1, logging=False)
    assert minimizer.minimum == test[
        'descent'], f"Gradient Descent broke on {test['num_inputs']} inputs {minimizer.minimum} should be equal to {test['descent']}"
